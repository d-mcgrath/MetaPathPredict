...
####### NOTE: n, x_j, and p_j will be required inputs;
########### NOTE: n, p_j AND x_j SHOULD be retrievable from present_and_missing_modules
...
## requires p_j, x_j, n to be columns in present_and_missing_modules
get_p.xj_k <- function(n, x_j, p_j) {
  k <- x_j:(n - 1)
  p.xj_k <- choose(k, x_j) * p_j^x_j * (1 - p_j)^(k - x_j)
  # NOTE: seq_along(x_j:(n - 1)) !!! DOES NOT WORK AS EXPECTED; SEQ_ALONG(4:7) = 1 2 3 4
  return(p.xj_k)
}
...
get_pj.k_betabinomial <- function(alpha, beta, y_j, n, m_j, x_j) { # k = seq_along(x_j:(n - 1)) # x_j should never equal n; if it does, the module step is complete/present
  ...
  A_j <- alpha + y_j
  B_j <- beta + (n * m_j) - y_j
  #pj.k <- ((gamma(A_j + B_j) * gamma(A_j + k) * gamma(B_j + n - k)) / (gamma(A_j) * gamma(B_j) * gamma(A_j + B_j + n))) # ADDED THE SUM() function here; before it was giving a distribution, not the sum...
